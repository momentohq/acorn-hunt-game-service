...
const increase = async (gameId, username, count) => {
  const momento = await getCacheClient(['super-abilities', 'leaderboard']);

  const incrementResponse = await momento.increment('super-abilities', `${gameId}-${username}`, count);
  let remaining = incrementResponse.value;
  if (incrementResponse.value > MAX_SUPER_ABILITIES) {
    await momento.set('super-abilities', `${gameId}-${username}`, `${MAX_SUPER_ABILITIES}`);
...
const decrease = async (gameId, username) => {
  ...
  const decreaseResponse = await momento.increment('super-abilities', `${gameId}-${username}`, -1);
